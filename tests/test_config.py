...
def test_database_connection(db_engine):
    """Verify that the test database is reachable and tables are created."""
    inspector = inspect(db_engine)
    tables = inspector.get_table_names()
    assert "raw_transactions" in tables
    assert "core_transactions" in tables


def test_sample_transaction_structure(test_sample_transaction):
    """Check that the fixture returns a non-empty list of dicts."""
    assert isinstance(test_sample_transaction, list)
    assert len(test_sample_transaction) > 0
    first = test_sample_transaction[0]
